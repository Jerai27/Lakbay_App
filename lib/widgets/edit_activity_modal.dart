...
class _EditActivityModalState extends State<EditActivityModal> {
  ...
  void initState() {
    ...
    if (widget.activity != null) {
      ...
      selectedPeriod = (timeText.contains('PM') || timeText.contains('pm')) ? 'PM' : 'AM';
    } else {
      selectedDay = 'Day 1';
      selectedPeriod = 'AM';
    }
  ...
  void _saveActivity() {
    ...
    if (_isTimeConflict(selectedDay, timeController.text, selectedPeriod)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('⚠️ Time conflict! An activity already exists at $selectedDay ${timeController.text} $selectedPeriod'),
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Invalid cost format')),
      );
    }
  }
